...
from verl.trainer.ppo.diversity_judge import fetch_all_cluster_assignments_async
from verl.trainer.ppo.polyepo_utils import (
    cluster_diversity_metrics,
    positive_advantage_metrics,
...
def compute_grpo_outcome_advantage(
    ...
    """
    Compute GRPO advantage from outcome rewards (one scalar reward per response).

    Returns:
        (final_advantages, final_returns, metrics)
            final_advantages: (bs, response_length) — GRPO-normalized
            final_returns:    (bs, response_length) — raw (un-normalized) rewards
            metrics:          dict of wandb-style keys, drained by the trainer
    """
    scores = token_level_rewards.sum(dim=-1)
    raw_scores = scores.clone()  # un-normalized rewards used in the returns tensor
    ...
    for idx in id2score:
        if len(id2score[idx]) <= 1:
            id2mean[idx] = torch.mean(torch.stack(id2score[idx])) if id2score[idx] else torch.tensor(0.0)
            id2std[idx] = torch.tensor(1.0)
        else:
    ...
    metrics: dict = {}
    metrics.update(success_rate_metrics(scores, id2indices))
    api_jobs = [
        {
            "context": inputs[id2indices[idx][0]],
            "responses": [responses[i] for i in id2indices[idx]],
            "prompt_id": idx,
        }
        for idx in sorted(id2indices)
    ]
    if api_jobs:
        with torch.no_grad():
    ...
    metrics.update(positive_advantage_metrics(advantages, id2indices))

    final_advantages = advantages.unsqueeze(-1) * response_mask
    final_returns = raw_scores.unsqueeze(-1) * response_mask
    return final_advantages, final_returns, metrics


@register_adv_est(AdvantageEstimator.POLYEPO)
def compute_polyepo_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    inputs: list[str],
    ...
    n = 4  # number of samples in each set in set RL
    scores = token_level_rewards.sum(dim=-1)
    raw_scores = scores.clone()  # un-normalized rewards used for logging
    ...
    for idx in group_indices_order:
        ...
        if len(batch_indices) < n:
            continue
        api_jobs.append({
            "context": inputs[batch_indices[0]],
            "responses": [responses[i] for i in batch_indices],
            "prompt_id": idx,
        })
    ...
    with torch.no_grad():
        for idx in group_indices_order:
            ...
            cluster_assignments_list = cluster_map[idx]
            if cluster_assignments_list is None or any(c is None for c in cluster_assignments_list):
                print(f"Warning: None detected in clusters for prompt {idx}. Defaulting to distinct.")
                cluster_assignments_list = list(range(1, group_size + 1))
            if len(cluster_assignments_list) != group_size:
                print(f"Warning: Mismatched cluster assignments for prompt {idx}. "
                      f"Expected {group_size}, got {len(cluster_assignments_list)}. Skipping prompt.")
                continue

            group_batch_indices = torch.tensor(batch_indices, device=device, dtype=torch.long)
            group_scores = scores[group_batch_indices]
            cluster_assignments_tensor = torch.tensor(cluster_assignments_list, device=device, dtype=torch.long)

            sampled_group_idx = torch.tensor(
                list(combinations(range(group_size), n)), device=device, dtype=torch.long
            )
            num_sets = sampled_group_idx.shape[0]
            original_batch_idx_for_sets = group_batch_indices[sampled_group_idx]
            ...
            set_diversity_tensor = torch.tensor(set_diversity_scores, device=device, dtype=scores.dtype)
            ...
            adv_sum.scatter_add_(0, indices_to_update, adv_values_to_add)
            returns_sum.scatter_add_(0, indices_to_update, ret_values_to_add)
            adv_count.scatter_add_(
                0,
                indices_to_update,
                torch.ones_like(indices_to_update, dtype=torch.float32),
            )

    poly_adv_flat = adv_sum / (adv_count + epsilon)
    poly_ret_flat = returns_sum / (adv_count + epsilon)
    ...
    if all_prompt_avg_diversities:
        metrics.update({
            "diversity/avg_of_set_avg_scores": torch.stack(all_prompt_avg_diversities).mean().item(),
            "diversity/avg_of_set_var_scores": torch.stack(all_prompt_var_diversities).mean().item(),
        })

    return poly_advantages, poly_returns, metrics
